...
const DropdownUser = () => {
  ...
  function roles(param: string | null) {
     if (param === 'ROLE_USER' || param === 'ROLE_STUDENT') return 'Фойдаланувчи';
     else return 'Guest'
  }
  ...
  return (
    <ClickOutside onClick={() => setDropdownOpen(false)} className="relative">
      ...
      {dropdownOpen && (
        <div
          className={`absolute right-0 mt-4 flex w-62.5 flex-col rounded-sm border border-stroke bg-[#6A9C89] shadow-default`}
        >
            <>
              <ul className="flex flex-col gap-5 border-b border-stroke px-6 py-4 dark:border-strokedark">
                <li>
                  <p
                    ...
                    {getMeData && getMeData.firstName + ' ' + getMeData.lastName}
                  </p>
                </li>
          ...
          <button
            className="flex items-center gap-3.5 px-6 py-4 text-sm font-medium duration-300 ease-in-out text-white   lg:text-base"
            onClick={() => {
